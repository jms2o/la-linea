...
export function PublicFooter() {
  return (
    <footer className="relative overflow-hidden bg-black text-white">
      ...
      <FadeIn className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="grid gap-6 lg:grid-cols-[1.3fr_0.8fr_0.9fr] lg:gap-8">
          <div>
          ...
          <div>
            ...
            <nav className="mt-2.5 grid gap-1.5 text-sm text-white/80">
              {shopLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
          ...
          <div>
            ...
            <div className="mt-2.5 grid gap-2 text-sm text-white/70">
              {contactRows.map((row) => (
                <a
                  key={row.label}
                  href={row.href}
                ...
                >
                  <span className={cn("grid h-6 w-6 place-items-center rounded-full text-white", row.bg)}>
                    <row.icon size={11} />
                  </span>
              ...
              <span className="flex items-center gap-2">
                <span className="grid h-6 w-6 place-items-center rounded-full bg-[var(--blue)] text-white">
                  <Mail size={11} />
                </span>
                ventas@lalinea.local
              </span>
              <span className="flex items-center gap-2">
                <span className="grid h-6 w-6 place-items-center rounded-full bg-[var(--red)] text-white">
                  <MapPin size={11} />
                </span>
                Chihuahua, Mexico
              </span>
        ...
        <div className="mt-6 flex flex-col items-center gap-3 border-t border-white/10 pt-4 text-xs text-white/40 sm:flex-row sm:justify-between">
